...
TARGET_INDUSTRIES = {
    '地产', '建筑', '银行', '保险', '运输', '电力', '电信',
    '医药', '消费', '科技', '工业', '资源', '新能源'
}

USER_CATEGORY_META = {
    '地产': {'examples': '保利发展、招商积余', 'focus': '地产复苏与轻资产转型'},
    '建筑': {'examples': '中国建筑、中国中铁', 'focus': '逆周期调节、大基建投资'},
    '银行': {'examples': '工商银行、宁波银行', 'focus': '宏观流动性、资产质量'},
    '保险': {'examples': '中国平安、中国太保', 'focus': '投资收益、长端利率'},
    '运输': {'examples': '顺丰控股、宁沪高速', 'focus': '物流活跃度、防御性分红'},
    '电力': {'examples': '长江电力、三峡能源', 'focus': '确定性溢价、碳中和进度'},
    '电信': {'examples': '中国移动、中兴通讯', 'focus': '数据要素、AI算力基座'},
    '医药': {'examples': '迈瑞医疗、药明康德', 'focus': '医疗刚需、CXO全球化'},
    '消费': {'examples': '伊利股份、五粮液', 'focus': '居民购买力、品牌溢价'},
    '科技': {'examples': '中芯国际、金山办公', 'focus': '半导体国产化、办公AI化'},
    '工业': {'examples': '三一重工、中航沈飞', 'focus': '出海逻辑、军工景气度'},
    '资源': {'examples': '中国神华、紫金矿业', 'focus': '大宗商品价格、抗通胀属性'},
    '新能源': {'examples': '宁德时代、隆基绿能', 'focus': '成长赛道、制造业竞争力'},
}
...
CATEGORY_COLLAPSE_MAP = {
    ...
    '建筑装饰': '建筑', '建筑': '建筑',
    '银行': '银行',
    '非银金融': '保险', '保险': '保险', '证券': '保险', '金融': '保险',
    '交通运输': '运输', '运输': '运输',
    '公用事业': '电力', '电力': '电力', '环保': '电力',
    '通信': '电信', '电信': '电信',
    '医药生物': '医药', '医药': '医药',
    '食品饮料': '消费', '家用电器': '消费', '社会服务': '消费', '商贸零售': '消费',
    '纺织服饰': '消费', '轻工制造': '消费', '农林牧渔': '消费', '消费': '消费',
    '电子': '科技', '计算机': '科技', '传媒': '科技', '科技': '科技',
    '机械设备': '工业', '国防军工': '工业', '汽车': '工业', '工业': '工业', '综合': '工业',
    '基础化工': '资源', '有色金属': '资源', '钢铁': '资源', '煤炭': '资源',
    '石油石化': '资源', '建筑材料': '资源', '资源': '资源',
    '电力设备': '新能源', '新能源': '新能源',
    '支持服务': '消费', '食物饮品': '消费', '纸及纸制品': '消费', '能源储存装置': '新能源',
    '投资及资产管理': '保险', '工业零件及器材': '工业',
}

MAX_STOCKS_PER_CATEGORY = 3
TARGET_FINAL_COUNT = len(TARGET_INDUSTRIES) * MAX_STOCKS_PER_CATEGORY
...
INVALID_INDUSTRIES = {'', '未知', '其他', '综合企业', '其他行业', '未分类', '待定', 'n/a', 'N/A', '-'}
GENERIC_INDUSTRIES = {
    '非金融', '金融', '保险', '证券', '地产', '建筑', '运输', '电力', '电信', '医药', '消费',
    '科技', '工业', '石油天然气', '必需性消费', '非必需性消费', '资讯科技业', '金融业',
    '医疗保健业', '电讯业', '原材料业', '能源业'
}
...
INDUSTRY_KEYWORDS = [
    ('地产', ['地产', '物业开发', '物业投资', '房地产', 'reit']),
    ('建筑', ['建筑工程', '土木工程', '基建建设', '工程承包', '铁路建设', '交通建设',
              '路桥建设', '市政工程', '中铁', '铁建', '建工集团', '建设集团', '施工']),
    ('银行', ['银行', '商业银行', '城商行', '农商行', '农信社']),
    ('保险', ['保险', '寿险', '财险', '再保险', '证券', '券商', '期货', '信托', '融资租赁', '金融']),
    ('运输', ['航运', '集装箱', '海运', '船舶运输', '物流运输', '高速公路', '港口', '铁路运输', '运输', '机场', '快递']),
    ('电力', ['发电', '电力', '电网', '新能源发电', '清洁能源', '光伏电站', '太阳能发电', '风电', '水电', '核电', '燃气', '水务', '环保']),
    ('电信', ['电信', '通信网络', '运营商', '5g', '6g', '通信', '电讯', '光模块', '光纤', '光缆']),
    ('医药', ['医药', '制药', '生物科技', '医疗器械', '医疗集团', '医院', '健康服务', '疫苗', 'cxo', '体外诊断']),
    ('消费', ['食品', '饮料', '零售', '消费品', '家电', '酒店', '味精', '氨基酸', '农产品加工', '餐饮', '烟草', '出版', '图书', '教育', '母婴']),
    ('科技', ['软件', '信息技术', 'it服务', '数字化', '云计算', '人工智能', '互联网', '半导体', '芯片', '数据中心', '科技集团', '游戏', '影视', '数字媒体']),
    ('工业', ['制造', '工业', '装备', '机械', '工程服务', '造纸', '通号', '航空制造', '汽车零部件', '零部件', '轴承', '包装', '军工', '中车']),
    ('资源', ['煤炭', '焦煤', '动力煤', '石油', '天然气', '油气', '炼化', '油田', '海油', '化工', '有色', '钢铁', '建材', '水泥', '玻璃']),
    ('新能源', ['光伏设备', '光伏制造', '风电设备', '风电整机', '锂电', '电池', '储能', '逆变器', '电网设备', '新能源车产业链']),
]
...
def normalize_industry(name: str) -> str:
    ...
    if not n:
        return n
    if n in TARGET_INDUSTRIES:
        return n
    if n in INDUSTRY_CANONICAL_MAP:
        n = INDUSTRY_CANONICAL_MAP[n]
    if n in CATEGORY_COLLAPSE_MAP:
        return CATEGORY_COLLAPSE_MAP[n]

    n_lower = n.lower()
    for target, words in INDUSTRY_KEYWORDS:
        if any(w.lower() in n_lower for w in words):
            return CATEGORY_COLLAPSE_MAP.get(target, target)
    return CATEGORY_COLLAPSE_MAP.get(n, n)
...
def is_invalid_industry(name: str) -> bool:
    ...
    if n in TARGET_INDUSTRIES:
        return False
    if n in INVALID_INDUSTRIES or n in GENERIC_INDUSTRIES:
        return True
    if '其他' in n:
        return True
    return False
...
def pick_best_industry(items):
    ...
    def score(it):
        ...
        generic_penalty = 5 if name in INVALID_INDUSTRIES or name in GENERIC_INDUSTRIES else 0
...
def resolve_industry(company: dict) -> str:
    ...
    if stock_code in MANUAL_INDUSTRY_MAP:
        return normalize_industry(MANUAL_INDUSTRY_MAP[stock_code])
    ...
    if fs_type in FS_TYPE_INDUSTRY_MAP:
        return normalize_industry(FS_TYPE_INDUSTRY_MAP[fs_type])
...
def select_top_n(df, n=TARGET_FINAL_COUNT, max_per_industry=MAX_STOCKS_PER_CATEGORY):
    """按用户定义的13类筛选，每类最多3家。"""
    result = []
    industry_count = {}
...
def generate_html_report(df_final, position_data, date_str, rating_dist, nine_info, config):
    ...
    category_meta_rows = ''.join([
        f"<tr><td>{cat}</td><td>{meta['examples']}</td><td>{meta['focus']}</td></tr>"
        for cat, meta in USER_CATEGORY_META.items()
    ])
...
<div class="card">
<h2>行业观察口径（13类，每类最多 {MAX_STOCKS_PER_CATEGORY} 家）</h2>
<table>
<tr><th>类别</th><th>代表公司</th><th>核心观察点</th></tr>
{category_meta_rows}
</table>
</div>
<div class="card">
